    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            ...
            if get_caller_address() != self.contract_owner.read() {
                panic!("Only owner can add stock");
            }
            //get a snapshot of the current stock
            let prev_stock = @self.storage_inventory.read(product);
            self.storage_inventory.write(product, *prev_stock + new_stock);
        }

        fn purchase(ref self: ContractState, product: felt252, quantity:
         u32) {
            // TODO:
            // * takes product and quantity
            // * subtracts quantity from stock in inventory
            // * anybody can call this
            let stock = self.storage_inventory.read(product);
            if stock < quantity {
                panic!("Out of stock");
            }
            self.storage_inventory.write(product, stock - quantity);
        }
        ...
        fn get_stock(self: @ContractState, product: felt252) -> u32 {
            ...
            return self.storage_inventory.read(product);
        }
